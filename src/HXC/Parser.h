...
static ASTNode* parseAfterID(ASTNode* lastNodes, Token* tokens, int* index, int size, FunCallPitchTable& pitchTable,
    ...
    PackedClassVarMem* varMem = nullptr;
    ...
    if (fromClass == nullptr) {
        ...
        if (!fromClassPtr) {
            setError(ERR_CANNOT_FIND_SYMBOL, node->token->line, fromClass);
            *err = 255;
            return NULL;
        }
        varMem = findVarMemberInClass(node->data.var.name, fromClassPtr, classTable);
        if (varMem) {
...
AFTER_COLON:
    ...
    if (*index < size - 1) {
        ...
        // 若 allowParseColon 为 false 且遇到冒号，直接返回（由外层循环处理）
        if (!allowParseColon && next->type == TOK_OPR_COLON) {
            (*index)++;
            return node;
        }
        if (*index < size - 1) {
    ...
    if (tokens[*index].type == TOK_OPR_LQUOTE) {
        ...
        if (isCast && (*index + 1 < size) && tokens[*index + 1].type == TOK_OPR_RQUOTE) {
            *index += 2;  // 跳过类型关键字和右括号
            ...
            // (int)a 和 (int)(a+b))
            ASTNode* inner =
                parsePrimary(tokens, index, size, pitchTable, table, outsideTable, localeScopeIndex, classTable, err);
...
ASTNode* parseExprRec(Token* tokens, int* index, int size, FunCallPitchTable& pitchTable, SymbolTable* table,
    ...
    while (*index < size) {
        ...
        // 递归解析右侧操作数
        ASTNode* rhs =
            parseExprRec(tokens, index, size, pitchTable, table, outsideTable, localeScopeIndex, classTable, err, prec + 1);
