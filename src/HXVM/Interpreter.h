...
inline int interpretInstruction(Instruction& inst, OpStack& opStack, char*& stack, ObjectCode& obj, int& instIndex,
...
#ifdef HX_DEBUG
        wprintf(LOG_LABEL L"通过栈顶存的地址加载偏移后的变量地址\n");
#endif
        if (opStack.top <= 0) {
            fwprintf(errorStream, ERR_LABEL L"栈中空荡荡的喵，因此不能OP_LOAD_VARIABLE_FROM_ADDRESS喵～\n");
            return -1;
        }
        // 获取栈顶元素（存储的是地址）
        _OpStack& topOp = opStack.opStack[opStack.top - 1];
        void* addr = nullptr;
        memcpy(&addr, topOp.value, sizeof(void*));
        uint32_t offest = 0;
        memcpy(&offest, inst.params[0].value, sizeof(uint32_t));
        addr = (char*)addr + offest;
        memcpy(topOp.value, &addr, sizeof(void*));
        topOp.type = TYPE_ADDRESS;
        topOp.size = sizeof(void*);
    ...
    case OP_STORE_VARIABLE_FROM_ADDRESS: {
...
#endif
        if (opStack.top <= 2) {
            fwprintf(errorStream, ERR_LABEL L"栈中空荡荡的喵，因此不能OP_STORE_VARIABLE_FROM_ADDRESS喵～\n");
            return -1;
        }
        opStack.top--;
        ...
        if(addr == nullptr) {
            fwprintf(errorStream, ERR_LABEL L"执行OP_STORE_VARIABLE_FROM_ADDRESS喵～☆发现空指针喵～崩溃了就\n   受  着  呗\n");
            return -1;
        }
        addr = (char*)addr + offest;
